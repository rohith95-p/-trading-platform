...
class TestNewsClassifier:
    ...
    def test_classifier_initialization_requires_key(self, monkeypatch):
        """Classifier should fail fast if NVIDIA_API_KEY isn't set."""
        monkeypatch.delenv("NVIDIA_API_KEY", raising=False)
        with pytest.raises(ValueError):
            NewsClassifier(api_key=None)

    def test_classifier_cache_key(self):
        classifier = NewsClassifier(api_key="test-key", redis_url="redis://localhost:6379")
        key1 = classifier._generate_cache_key({"title": "t", "content": "c", "url": "u"})
        key2 = classifier._generate_cache_key({"title": "t", "content": "c", "url": "u"})
        assert key1 == key2

    @pytest.mark.asyncio
    async def test_classifier_with_mock_api(self):
        """Async classify() should parse JSON response and emit signals."""
        classifier = NewsClassifier(api_key="test-key", redis_url="redis://localhost:6379")
        classifier.redis_client = AsyncMock()
        ...
        classifier.client.chat.completions.create = AsyncMock(return_value=mock_message)

        result = await classifier.classify(
            {"title": "t", "content": "c", "source": "rss", "url": "u"}
        )
    ...
    @pytest.mark.asyncio
    async def test_classifier_error_returns_neutral(self):
        classifier = NewsClassifier(api_key="test-key", redis_url="redis://localhost:6379")
        classifier.redis_client = AsyncMock()
        classifier.redis_client.get.return_value = None
        classifier.client.chat.completions.create = AsyncMock(side_effect=Exception("boom"))
