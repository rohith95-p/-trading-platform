"""Phase 3 WIDE-STOP variant (user-directed: ~$15/trade, breathing room) — the hypothesis Phase 1 actually generated.
...
TP_MULTS = {1.5: [2.0, 3.0, 4.5], 2.0: [3.0, 4.0]}
...
def run(only_rule=None, out_override=None):
    ...
    out = out_override or os.path.join(RUNS_ROOT, f"{datetime.now(timezone.utc):%Y%m%d-%H%M%S}_phase3_tight_stops")
    ...
    results = {}
    magic = 4000
    for rule_name, cid in ([(only_rule, RULES[only_rule])] if only_rule else RULES.items()):
        rule = lib[cid].rule
    ...
    with open(os.path.join(out, f"results_{only_rule or 'all'}.json"), "w", encoding="utf-8") as fh:
...
if __name__ == "__main__":
    import sys
    run(sys.argv[1] if len(sys.argv)>1 else None, sys.argv[2] if len(sys.argv)>2 else None)
