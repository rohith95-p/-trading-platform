"""Path-to-$20/day modeling. Reruns portfolio_v4's 4 legs at several fixed
lot sizes (checking linearity + margin binding), then builds a day-by-day
equity curve at fixed 0.01 lots AND under 3 lot-step-up policies, using the
real trade sequence from the 0.01-lot run scaled by each policy's tier
multiplier at the time of each trade. Writes JSON results for the report.
"""
import json
...
def run_legs(fixed_lots):
    """Run all 4 legs isolated (breaker off) at a given fixed lot size."""
    all_trades = []
...
def replay_shared_account(all_trades, start_bal=START_BAL):
    """Chronological replay on one shared account, real 6% daily breaker (IST calendar day)."""
    balance = start_bal
    min_bal = start_bal
    peak = start_bal
    max_dd = 0.0
...
# ---------------------------------------------------------------------------
# 1. Linearity + margin-binding check across fixed lot sizes
# ---------------------------------------------------------------------------
print("=" * 70)
print("STEP 1: fixed-lot sweep -- linearity & margin binding check")
print("=" * 70)
sweep_results = {}
for lots in (0.01, 0.02, 0.03, 0.05, 0.08, 0.10, 0.15, 0.20):
    ...
    sweep_results[lots] = {
        ...
        "per_day_avg": rep["per_day_avg"],
    }
    print(f"lots={lots:.2f}  net=${rep['net']:.2f}  pf={rep['pf']:.3f}  "
...
# ---------------------------------------------------------------------------
# 2. Base 0.01-lot trade sequence, used for the step-up simulations
# ---------------------------------------------------------------------------
print()
print("=" * 70)
print("STEP 2: base 0.01-lot trade sequence for step-up simulation")
print("=" * 70)
base_trades, _ = run_legs(0.01)
base_rep = replay_shared_account(base_trades, start_bal=START_BAL)
print(f"BASE (0.01 lots fixed): {base_rep['trades_kept']} trades, net=${base_rep['net']:.2f}, "
      f"pf={base_rep['pf']:.3f}, min_bal=${base_rep['min_bal']:.2f}, max_dd={base_rep['max_dd_pct']:.1f}%, "
      f"span={base_rep['calendar_span_days']}d, $/day={base_rep['per_day_avg']:.2f}")

# Save the kept trade list (per-0.01-lot pl, entry/exit ts, day) for step-up modeling
kept_serializable = [
    {"entry_ts": e, "exit_ts": x, "leg": n, "pl_001": pl, "day": str(d)}
    for (e, x, n, pl, bal, d) in base_rep["kept"]
]
with open("scratch_base_trades.json", "w") as f:
    json.dump(kept_serializable, f, indent=2)

print(f"\nSaved {len(kept_serializable)} trades to scratch_base_trades.json")
print(f"Saved lot sweep to scratch_sweep.json")
